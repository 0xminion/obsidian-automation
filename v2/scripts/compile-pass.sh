...
# ============================================================================
# Scans wiki/entries/, wiki/concepts/, wiki/mocs/ and performs a
...
# 4. Wiki index: rebuilds config/wiki-index.md from scratch.
...
entry_count=$(find "$VAULT_PATH/wiki/entries" -name '*.md' 2>/dev/null | wc -l)
concept_count=$(find "$VAULT_PATH/wiki/concepts" -name '*.md' 2>/dev/null | wc -l)
moc_count=$(find "$VAULT_PATH/wiki/mocs" -name '*.md' 2>/dev/null | wc -l)
...
You are working with notes in wiki/entries/, wiki/concepts/, and wiki/mocs/.
...
Scan wiki/concepts/ for concepts that:
...
For each existing MoC in wiki/mocs/:
...
Rebuild 'config/wiki-index.md' from scratch:
- Start with the header (keep the existing intro text)
- List every Entry note from wiki/entries/ with a 1-sentence summary
  Format: - [[EntryName]]: <1-sentence summary> (entry)
- List every Concept note from wiki/concepts/ with a 1-sentence summary
