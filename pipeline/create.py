...
# ─── Template-based file generation (Rec 1) ──────────────────────────────────
...
def generate_source_content(
    ...
    author = _esc(extracted.get("author", ""))
...
{other_takeaways if other_takeaways else "- None yet."}
...
def generate_entry_insights(
    ...
    """
    content = extracted.get("content", "")[:6000]
    prompt = f"""You are a knowledge extraction agent. Read the content below and write TWO sections for an Obsidian wiki entry.

CONTENT:
{content}

Write EXACTLY these two sections — nothing else:

## Summary
(2-4 sentences summarizing the key ideas)

## Core insights
(Numbered list of 3-5 key insights, each with brief explanation)

Be precise. No filler. No meta-commentary."""
    ...
    try:
        ...
        log.warning("Insight agent failed for %s: %s", plan.title, e)
    return ""
