...
class TestBuildPlanPrompt:
    """Verify build_plan_prompt includes tag_candidates for agent curation."""

    def test_prompt_includes_tag_candidates(self, tmp_path):
        cfg = _make_config(tmp_path)
        src = _make_source(
            title="Bitcoin Surges Past $100K",
            url="https://x.com/cryptonews/status/123",
            content="Bitcoin surged past $100,000 as institutional adoption accelerated. "
                    "Ethereum smart contracts dominate DeFi. Solana ecosystem growing.",
        )
        manifest = _make_manifest(src)
        prompt = build_plan_prompt(manifest, {}, cfg)
        assert "tag_candidates" in prompt
        # Should contain heuristic candidates
        assert "bitcoin" in prompt.lower() or "crypto" in prompt.lower()
        # Should mention curation rules
        assert "CURATE" in prompt or "curate" in prompt.lower()

    def test_prompt_candidates_are_json(self, tmp_path):
        cfg = _make_config(tmp_path)
        src = _make_source(
            content="Machine learning and deep learning transform AI research. "
                    "Neural networks process natural language. GPT-4 leads the field.",
        )
        manifest = _make_manifest(src)
        prompt = build_plan_prompt(manifest, {}, cfg)
        # Extract the tag_candidates line and verify it's valid JSON
        for line in prompt.split("\n"):
            if "tag_candidates" in line:
                json_part = line.split(":", 1)[1].strip()
                candidates = json.loads(json_part)
                assert isinstance(candidates, list)
                assert len(candidates) > 0
                break
        else:
            pytest.fail("tag_candidates not found in prompt")
